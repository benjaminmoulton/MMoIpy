...
if __name__ == "__main__":
    ...
    axs_cgs[0].legend()
    #   Ims
    axs_Ims.grid(which="major",lw=0.6,ls="-",c="0.75")
    # axs_Ims.grid(which="major",lw=0.6,ls="-",c="0.75")
    # axs_Ims.grid(which="major",lw=0.6,ls="-",c="0.75")
    axs_Ims.plot(RT  ,Is [0,0,0],"k-",label=r"$I_{xx}$")#uslbl)
    # axs_Ims[0].plot(RT  ,Is [1,0,0],"k--",label=lnlbl)
    axs_Ims.plot(RT  ,Is [0,1,1],"-",c="0.3",label=r"$I_{yy}$")#uslbl)
    # axs_Ims[1].plot(RT  ,Is [1,1,1],"k--",label=lnlbl)
    axs_Ims.plot(RT  ,Is [0,2,2],"-",c="0.6",label=r"$I_{zz}$")#uslbl)
    # axs_Ims[2].plot(RT  ,Is [1,2,2],"k--",label=lnlbl)
    axs_Ims.plot(RTSW,IW   [0,0],"w.",mec="k",mew=0.5,label=SWlbl)
    axs_Ims.plot(RTSW,IW   [1,1],"w.",mec="0.3",mew=0.5)#,label=SWlbl)
    axs_Ims.plot(RTSW,IW   [2,2],"w.",mec="0.6",mew=0.5)#,label=SWlbl)
    ...
    # axs_Ims.set_ylabel(r"$I_{xx}$")
    # axs_Ims.set_ylabel(r"$I_{yy}$")
    # axs_Ims.set_ylabel(r"$I_{zz}$")
    axs_Ims.set_ylabel("Moments of Inertia")
    axs_Ims.set_xlabel("Taper Ratio, $c_t/c_r$")
    axs_Ims.set_xlim((RT[0],RT[-1]))
    axs_Ims.legend()
    #   Ips
